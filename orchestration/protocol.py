...
"""协议版本——修改模板文本即升版（协议 §6）。
v1.2：增补执行侧两项声明——副作用任务的幂等责任（行为约束 5）与
目标及契约范围内的取舍裁定权（行为约束 6）。"""

# ---------------------------------------------------------------------------
# 完整版模板：全量字段说明 + few-shot 正/反例（协议 §3 + §7.4）
# ---------------------------------------------------------------------------
PROTOCOL_PROMPT_FULL = """# Agent 执行协议 v1.2
...
# ---------------------------------------------------------------------------
PROTOCOL_PROMPT_SIMPLE = """# Agent 执行协议 v1.2（简化版）
...
- 任务声明副作用（constraints.side_effects 非 none）时须自行保证幂等——同一任务可能被重复下发
- 目标与契约范围内的全部取舍由你自行裁定，含价值性取舍；不设征询通道，不等待外部决策
