...
> **边界澄清**：规划层的**目标拆解**（目标 → DAG）由框架直接调用自己的底层 LLM，**不经过本协议**——拆解是框架内部组件，不在"框架 ↔ agent"这条边界上。本协议约束的只有与 agent 通信的这一面。
>
> 与之相对，治理层的**反思/判定**（最终交付 × 原始目标 → 判定结论）**就是本协议的一次普通 `task_request`**——判定能力同样是"问出来的能力"（`judge` / `reviewer` 由 info_request 采集），判定 agent 与执行 agent 同构、零变更；因此**协议不新增消息类型**（无 judge_request 之类），判定输出复用结果契约、受 §7.2 output_schema 强校验与 §7.3 解析重试约束。
...
> **真实值由执行层适配器在解析完成后从模型 API 响应捕获并回填**，agent 自报仅作兜底、
> 且以 API 真实值为准。简化版模板下
...
> 此模板即协议全部内容。框架侧以**完整版**与**简化版**两个模板维护（档位按 agent model 配置，见 §7.8）；每次请求 = 模板 + 具体请求 JSON 拼接后发给 agent。
...
## 7. 稳定性兜底：解耦在协议面，稳定在解析组件

> 核心原则：agent 侧保持零适配（P3），**所有稳定性风险由框架解析组件吸收**。
...
| # | 风险 | 兜底措施 | 所在层 |
|---|---|---|---|
| R1 | **格式漂移**：字段缺失 / 类型错 / 转义坏 / 幻觉加字段 | 双层校验 + 解析失败重试 + few-shot 示例（§7.2~7.4） | 执行层 |
| R2 | **语义漂移**：capability 误报、side_effects 乱填、同一模板不可复现 | 交叉校验 + 计入审计（§7.5） | 治理层 |
| R3 | **协议与业务数据同通道**：业务内容被 agent 误读为协议指令（间接注入面） | 分隔标记 + 解析时丢弃非 JSON 内容（§7.6） | 执行层 |
| R4 | **可观测性弱**：日志 / 对账 / 成本核算依赖事后解析 | `request_id` 强制回带 + 结构化日志（§7.7） | 执行层 |
...
- 解析组件只提取响应中的 JSON，丢弃一切非 JSON 内容——即使业务数据被污染也不影响协议
...
- **工程要求**：JSON 解析组件（提取 + Schema 校验 + 重试）必须是框架内**最严格、测试最全**的模块，没有之一——它是整个协议稳定性的地基
