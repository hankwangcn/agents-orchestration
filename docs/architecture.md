...
> 版本：v1.0
> 更新日期：2026-09-24
> 状态：四阶段实现完成，六层详细设计完成；481/481 测试通过
...
| Agent（外部） | 接收请求并返回符合结果契约的 JSON，不为框架做任何适配；目标及契约范围内的取舍（含价值性取舍）由其自行裁定，框架不设征询通道（协议 §3 行为约束） | 提示词 → JSON 响应 |

**治理层（GOVERN）**

| 模块 | 职责 | 输入 → 输出 |
|---|---|---|
| 结果审计器 | 对账事实：任务状态与结果契约的一致性、缺失契约、迟到结果、分配（降级与风险）、剪枝、副作用声明与实际报告的交叉校验、错误模式归集。只读、纯函数、结论可重复（可重放） | 结果契约集 → 审计报告 |
...
重试为任务级动作，不区分副作用声明——同一任务在一次派发内最多执行 `重试次数 + 1` 次（默认 3 次）。因此副作用任务的幂等责任归执行侧，并由协议模板显式声明（协议 §3 行为约束第 5 条、§8.4）；框架侧不对其重复执行设拦截。§5.5 的 A+B 策略仅约束断点恢复路径，与派发内重试路径口径不同。
...
> A+B 策略仅适用于断点恢复路径；派发内的失败重试不区分副作用声明（见 §5.1），幂等责任归执行侧。
...
**人读版为按需渲染**：人读投影是运行存档的读时投影，为纯函数——同一份存档渲染出同一份视图，可重放、零成本、不含大模型调用。人读版不默认生成、不落盘。理由与"源 Markdown → HTML 发布副本"一致：存档为唯一权威来源，视图是它的纯函数；若默认全量生成，将引入第二份真相，需仲裁何者为权威，必然导致分叉。
...
- [x] **运行存档归档闭环、Web 页面、人读投影与叙述摘要**（2026-09-24）——背景：接入层此前仅有 API 网关与 CLI 轻量客户端，且运行存档只写不读——报告落盘后无读回路径（进程重启后已完成运行的报告无法获取），无过程时序（仅存终态，Web 无法回放执行过程），无运行枚举（换进程后无法发现已有运行）；接入层 Web 形态曾于早期被放弃，本次按要求重建。① 归档归属定为持久化底座（非学习层与治理层）：判据与失败处理器一致——审计、成本、判定与学习层均需读写该档案，若置于学习层则治理层反向依赖学习层获取输入，导致层序倒置；治理层为生产者之一，学习层为消费者，数据流为环形而非两级传递。② 过程以事件流记录：状态变更（启动、派发、完成、剪枝、丢弃迟到结果、取消、恢复、收尾）逐条落盘（过程事件表，与结构化日志共用同一处调用点与同一套事件名），过程可精确回放。③ 报告读回与运行枚举：底座补充报告读回与运行枚举能力（`GET /api/runs` / `ao runs`），进程重启后报告与运行列表仍可读取；网关报告端点增加底座读回。④ Web 页面（服务端渲染、同源、零构建、零跨域）：运行列表 `GET /` 与详情 `GET /runs/{id}`（详情含过程时间线、任务与结果、分配记录、失败传播、治理与学习）；人读版是运行存档的读时投影——按需渲染、纯函数（同一份存档渲染同一份视图，可重放、零成本）、不落盘为第二份真相（定义同"源 Markdown → HTML 发布副本"）；新增确定性投影端点 `GET /api/runs/{id}/view` 与 CLI 子命令 `ao view`。⑤ 叙述摘要（非确定，唯一例外）：以一段文字概括得失属大模型产出，仅支持显式触发（`POST /api/runs/{id}/narrative` / `ao narrate`），单独成节、标注来源（模型与生成时间）、写入独立表留档，绝不默认生成，也不混入确定性报告；未注入叙述引擎时端点返回 400，确定性投影始终可用。新增 33 项测试（运行存档 10、人读投影与转义 10、网关存档与 Web 9、CLI 存档出口 4），累计 468/468 测试通过（多种散列种子复跑恒定），见 §10。

**收尾与一致性（2026-09-24）**

- [x] **审计输入封闭化与可复现性术语校正**（2026-09-24）——① 审计输入封闭化：审计器构造时接收注册表实例并持有，而该引用在全模块无任何读取点，属声明与消费不一致的字段（与已清理的同步调度器退避参数同类）；现移除该参数与持有字段，审计输入严格封闭于调度收尾报告，"输入封闭 = 仅调度报告"由实况升为结构保证，调用点是否传入外部运行态均不影响结论。② 可复现性术语校正：原表述将性质与其推论混为一谈——审计的性质是引用透明（输入封闭、无副作用），"结论可复跑"是由该性质推出的结果；现统一为"只读、纯函数、结论可重复（可重放）"，与判定的对照面相应表述为"非确定、有成本、不可重放"，同层其他组件与人读投影的相同属性一并统一。新增 3 项回归用例（构造器不接收外部运行态、同一报告重复审计结论逐字节一致、审计不写入被审计报告）。累计 471/471 测试通过（多种散列种子复跑恒定），见 §10。
- [x] **执行侧职责声明：副作用任务的幂等责任与取舍裁定权归执行侧（协议面声明）**（2026-09-24）——背景：① 派发内的失败重试不查询副作用声明，对全部任务一律重试（默认重试 2 次，即最多执行 3 次），与断点恢复路径的 A+B 策略（声明副作用的任务不自动重派）口径不一致；任务失败时若副作用已实际发生，重试将重复该副作用。② 任务执行中需使用方裁决的情形此前无处置口径：可选路径为执行侧自主裁定或经框架征询，而后者要求框架持有任务的执行中间态。定标：两项职责一并明确交由执行侧，框架侧维持现状（零结构改动），并由协议模板显式声明。实现：完整版模板行为约束新增第 5 条（声明 `side_effects` 非 none 的任务须自行保证幂等或自行去重，理由为同一任务可能被重复下发——失败重试与断点恢复）与第 6 条（目标与契约范围内的全部取舍由执行侧自行裁定，含价值性取舍；框架不提供征询通道，不得以待外部决策为由中止执行或返回失败，所作取舍随 output 给出）；简化版模板同步声明两项义务；协议版本 v1.1 → v1.2（协议 §6：修改模板文本即升版本号）。§5.1 与 §5.5 补记两条重试路径的口径差异；协议 §7.8 补记简化版不因字段裁剪而豁免义务，协议 §8 补记框架不提供征询通道及其与单轮封闭性的关系。裁定权下放后，框架不承担"取舍是否符合使用方偏好"的判定责任，仅保留结构可判定项（结果契约对账、副作用声明一致性），事后纠偏由运行级判定（advisory）与后继运行承担。新增 5 项回归（模板幂等声明、版本号与模板声明一致、派发内重试不因副作用声明而收敛）。
...
测试覆盖重点：依赖分析（拓扑分层、并行前沿、可达性、成环与引用校验）、层职责切分（资源统计器与资源协调器）、解析组件（信封校验与输出结构强校验、重试容错）、剪枝算法（反向可达、多交付点语义）、并发竞态、速率限制、治理三项（审计 / 成本 / 学习）、审计输入封闭性（引用透明、结论可重放）、网关生命周期、断点恢复（A+B 策略）、CLI 命令与请求构造、交互式会话、进程内适配器、配置批量注册、采集超时、学习层闭环（证据分级、经验库落盘与跨运行聚合、复现门槛、提示词注入与客观 - 判定分离）、运行存档（事件流、报告读回、运行枚举、叙述留档、级联清理与底座退化）、人读投影（确定性、结构、转义、运行中投影、HTML 渲染）、网关存档与 Web、CLI 存档出口、执行侧职责声明（模板取舍裁定范围与提示词版本一致性）。当前累计 481/481 通过。
